    ...
    <main>
        <article class="prose mx-auto">
            ...
            <ContentDoc :query=query v-slot="{ doc }">
                <h3>Dissertation</h3>
                <ul>
                    <ol v-for="(dissertation, index) in doc.dissertation" v-bind:key="dissertation.id">
                ...
                <ul>
                    <ol v-for="(journal, index) in doc.journals" v-bind:key="journal.id">
                        <span class="w-8 inline-block">{{ index + 1 }}.</span>
                        <span>{{ journal.authors }}</span>,&nbsp;
                        <span>"{{ journal.title }}</span>",&nbsp;
                        <span inline-block italic font-semibold>{{ journal.journal }}</span>&nbsp;
                        <span>({{ journal.year }})</span>.
                    </ol>
                </ul>
                <h3>Books</h3>
                <ul>
                    <ol v-for="(book, index) in doc.books" v-bind:key="book.id">
                        <span class="w-8 inline-block">{{ index + 1 }}.</span>
